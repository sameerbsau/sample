...
public class DevGeniusPortal2 {
    ...
    public static void main(String[] args) {
        ...
        Map<String, Double> result1 = employeeList.stream().collect(Collectors.groupingBy(Employee::getGender, Collectors.averagingInt(Employee::getAge)));
        System.out.println(result1);

        //Get the Names of employees who joined after 2015.

        employeeList.stream().filter(r -> r.getYearOfJoining() > 2015).map(Employee::getName).forEach(System.out::println);

        //Count the number of employees in each department
        result = employeeList.stream().collect(Collectors.groupingBy(Employee::getDepartment, Collectors.counting()));
        System.out.println(result);

        //Find out the average salary of each department

        result1 = employeeList.stream().collect(Collectors.groupingBy(Employee::getDepartment, Collectors.averagingDouble(Employee::getSalary)));
        System.out.println(result1);
        ...
        employeeList.stream().collect(Collectors.groupingBy(Employee::getDepartment)).forEach((department, empList) -> {
        ...
        employeeList.stream().sorted(Comparator.comparingInt(Employee::getYearOfJoining)).map(Employee::getName)
